...
public class ImportData extends BasicSpringTest {
	...
	@Autowired
	private SockInfoService sockService;

	@Test
	public void testImportDataToDB() {
		File file = new File("D:/workroom/tools/sock/stockdata");
		if (file.isDirectory()) {
			...
			if (children != null) {
				for (File child : children) {
					...
					try {
						readLines = Files.readLines(child, Charset.forName("GBK"));
						if (CollectionUtils.isEmpty(readLines)) {
							...
							for (String readLine : readLines) {
								if (!readLine.contains(";")) {
									...
									String idSuffix = tokenizer.nextToken();
									if (id.endsWith(idSuffix)) {
										SockInfo sockInfo = new SockInfo();
										sockInfo.setId(id);
										String name = tokenizer.nextToken();
										sockInfo.setName(name);
										sockService.insert(sockInfo);
									}
									continue;
								}
								StringTokenizer tokenizer = new StringTokenizer(readLine, ";");
